...
class XMLViewer:
    def __init__(self, root):
        ...
        try:
            # Highlight tag for matches in notes editor
            self.note_text.tag_configure('matchhl', background='#FFF4C2')
        except Exception:
            pass
        ...
        try:
            ...
            try:
                self._preview_text.tag_configure('matchhl', background='#FFF4C2')
            except Exception:
                pass
        ...
        self.note_text.bind('<KeyRelease>', lambda e: (_schedule_preview_update(), self._apply_notes_highlight_safe()), add=True)
        ...
        self.scope_combo = ttk.Combobox(
            ...
            values=[
                ...
                'value', 'reset', 'address', 'offset', 'notes', 'any'
            ]
        )
        ...
        try:
            ...
            # Also highlight matches inside notes editor
            self.search_entry.bind('<KeyRelease>', lambda e: self._apply_notes_highlight_safe(), add=True)
            self.case_var.trace_add('write', lambda *a: self._apply_notes_highlight_safe())
            self.exact_var.trace_add('write', lambda *a: self._apply_notes_highlight_safe())
            self.regex_var.trace_add('write', lambda *a: self._apply_notes_highlight_safe())
            self.and_terms_var.trace_add('write', lambda *a: self._apply_notes_highlight_safe())
        except Exception:
            pass
    ...
    def _apply_notes_highlight_safe(self):
        try:
            self._apply_notes_highlight()
        except Exception:
            pass

    def _apply_notes_highlight(self):
        """Highlight current search query terms inside the notes Edit and Preview text widgets."""
        # Helper to highlight in any Text-like widget
        def highlight_in_text_widget(w):
            try:
                ...
                try:
                    ...
                    prev_state = None
                try:
                    w.tag_remove('matchhl', '1.0', tk.END)
                except Exception:
                    pass
                ...
                def add_span(start_idx, end_idx):
                    try:
                        w.tag_add('matchhl', start_idx, end_idx)
                    except Exception:
                        pass
                if use_regex:
                    try:
        ...
        try:
            ...
            raw_query = ''
        if not raw_query:
            # Still clear any previous highlights
            try:
                self.note_text.tag_remove('matchhl', '1.0', tk.END)
            except Exception:
                pass
            try:
                if self._preview_text is not None:
                    prev_state = self._preview_text.cget('state')
                    if prev_state == 'disabled':
                        self._preview_text.configure(state='normal')
                    self._preview_text.tag_remove('matchhl', '1.0', tk.END)
                    if prev_state == 'disabled':
        ...
        try:
            ...
            exact = False
            use_regex = False
        # Apply to Edit tab text widget
        highlight_in_text_widget(self.note_text)
        # Apply to Preview text fallback if available
        if getattr(self, '_preview_text', None) is not None:
            highlight_in_text_widget(self._preview_text)
    ...
    def perform_search(self):
        ...
        if not query:
            ...
            return
        matched = []
        # Cache note bodies per register to avoid repeated disk IO during one search
            ...
            if sc == 'notes':
                try:
                    ...
                    if key not in note_cache:
                        note_cache[key] = self._read_note_for(reg) or ''
                    nb = note_cache.get(key, '')
                except Exception:
                    nb = ''
                res = text_match(nb, local_query)
                return (not res) if negate else res
            # any
            base_match = (
        ...
        # Re-apply highlight in notes in case selection or content changed
        try:
            self._apply_notes_highlight_safe()
        except Exception:
            pass
    ...
    def _load_note_into_widget(self, reg: dict):
        ...
        try:
            ...
            pass
        # Apply search highlight inside notes after loading
        try:
            self._apply_notes_highlight_safe()
        except Exception:
            pass
    ...
    def _update_md_preview(self):
        ...
        if self._preview_text is not None:
            try:
                ...
                pass
        # After updating preview, re-apply notes highlight
        try:
            self._apply_notes_highlight_safe()
        except Exception:
            pass
