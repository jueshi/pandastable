        ...
        try:
            ...
            if filter_text:
                ...
                if not query_successful and query_part:
                    ...
                    # Initialize mask to include all rows initially
                    mask = pd.Series([True] * len(str_df), index=str_df.index)
                    
                    # Check if the query is a single exclusion term (starts with !)
                    if query_part.startswith('!'):
                        exclude_term = query_part[1:].strip('"\'')
                        if exclude_term:  # Only process if there's actually a term after !
                            # Exclude rows containing the term
                            escaped_term = re.escape(exclude_term)
                            mask = ~str_df.apply(
                                lambda x: x.str.contains(escaped_term, case=False, na=False, regex=True)
                            ).any(axis=1)
                            print(f"Excluding rows containing: '{exclude_term}' (escaped)")
                    else:
                        ...
                        if not search_terms:  # No valid terms found
                        ...
                        for term in search_terms:
                            ...
                            if term.startswith('!'):  # Exclusion term
                                ...
                                print(f"Including rows containing: '{term}' (escaped)")
