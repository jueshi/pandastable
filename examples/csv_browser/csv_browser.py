...
class CSVBrowser(tk.Tk):
    ...
    def filter_files_wrapper(self, *args):
        """Wrapper method to call FileTableUtils filter_files"""
        self.file_table_utils.filter_files(*args)
    ...
    def update_file_list(self):
        ...
        try:
            normalized_directory = self.file_table_utils.normalize_long_path(self.current_directory)
            ...
            try:
                for root, _, files in os.walk(normalized_directory):
                    try:
                        ...
                        normalized_root = self.file_table_utils.normalize_long_path(root)
        ...
        try:
            ...
            for row in selected_rows:
                if row < len(self.df):
                    ...
                    file_path = self.file_table_utils.normalize_long_path(original_path)
            ...
            try:
                ...
                try:
                    ...
                    if base_path:
                        ...
                        base_path = self.file_table_utils.normalize_long_path(base_path)
    ...
    def create_correlation_plots(self, data_df, target_column, corr_df, base_path):
        ...
        try:
            ...
            plots_dir = self.file_table_utils.normalize_long_path(f"{base_path}_plots")
    ...
    def update_file_dataframe(self):
        """Update the file list DataFrame with latest file information"""
        ...
        for file_path in self.csv_files:
            try:
                ...
                file_paths.append(file_path)
                sizes.append(self.file_table_utils.format_size(file_stat.st_size))
                modified_times.append(self.file_table_utils.format_date(file_stat.st_mtime))
                types.append(os.path.splitext(file_name)[1][1:].upper())
            except Exception as e:
                print(f"Error processing file {file_path}: {str(e)}")
                # Skip files that cause errors to maintain list lengths
                continue
        ...
        try:
            ...
            self.df['Modified'] = pd.to_datetime(self.df['Modified'], format='%Y-%m-%d %H:%M')
            self.df = self.df.sort_values('Modified', ascending=False)
            ...
            print(f"Created DataFrame with {len(self.df)} files")
        except Exception as e:
            print(f"Error creating DataFrame: {str(e)}")
            # Create an empty DataFrame with expected columns if processing fails
            self.df = pd.DataFrame(columns=[
                'Name', 'File_Path', 'Size', 'Modified', 'Type'
...
"""
    ...
    def _advanced_file_read(self, file_path):
        ...
        try:
            ...
            normalized_path = self.file_table_utils.normalize_long_path(file_path)
    ...
    def open_recent_directory(self, directory):
        ...
        try:
            ...
            # After line 3739: self.file_table_utils.update_file_list()
            # Add these lines:
            self.update_file_dataframe()
            self.setup_file_browser()
