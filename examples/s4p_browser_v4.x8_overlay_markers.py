            ...
            if show_tdr or show_pulse or show_impedance or show_impedance_time:
                ...
                for i, net in enumerate(networks):
                    ...
                    for marker_freq in self.markers:
                        ...
                        if overlay_mode:
                            # Overlay mode: place markers on shared axes
                            if show_mag and ax_mag is not None:
                                if show_sdd11:
                                    ax_mag.plot(f, sdd11_mag, 'ko')
                                    ax_mag.annotate(f'SDD11 {sdd11_mag:.2f} dB', (f, sdd11_mag),
                                                    xytext=(10, 10), textcoords='offset points')
                                if show_sdd21:
                                    ax_mag.plot(f, sdd21_mag, 'ko')
                                    ax_mag.annotate(f'SDD21 {sdd21_mag:.2f} dB', (f, sdd21_mag),
                                                    xytext=(10, 10), textcoords='offset points')
                                if show_s11:
                                    ax_mag.plot(f, s11_mag, 'ko')
                                    ax_mag.annotate(f'S11 {s11_mag:.2f} dB', (f, s11_mag),
                                                    xytext=(10, 10), textcoords='offset points')
                                if show_s21:
                                    ax_mag.plot(f, s21_mag, 'ko')
                                    ax_mag.annotate(f'S21 {s21_mag:.2f} dB', (f, s21_mag),
                                                    xytext=(10, 10), textcoords='offset points')
                                # S33/S43 if enabled and available
                                if self.plot_s33_var.get() and net.s.shape[1] >= 3:
                                    s33_mag = 20*np.log10(np.abs(net.s[idx, 2, 2]))
                                    ax_mag.plot(f, s33_mag, 'ko')
                                    ax_mag.annotate(f'S33 {s33_mag:.2f} dB', (f, s33_mag),
                                                    xytext=(10, 10), textcoords='offset points')
                                if self.plot_s43_var.get() and net.s.shape[1] >= 4:
                                    s43_mag = 20*np.log10(np.abs(net.s[idx, 3, 2]))
                                    ax_mag.plot(f, s43_mag, 'ko')
                                    ax_mag.annotate(f'S43 {s43_mag:.2f} dB', (f, s43_mag),
                                                    xytext=(10, 10), textcoords='offset points')
                                # Arbitrary Sij markers
                                if getattr(self, 'custom_sij', None):
                                    for (ii, jj) in self.custom_sij:
                                        if 1 <= ii <= net.s.shape[1] and 1 <= jj <= net.s.shape[2]:
                                            sij_mag = 20*np.log10(np.abs(net.s[idx, ii-1, jj-1]))
                                            ax_mag.plot(f, sij_mag, 'ko')
                                            ax_mag.annotate(f'S{ii}{jj} {sij_mag:.2f} dB', (f, sij_mag),
                                                            xytext=(10, 10), textcoords='offset points')
                            if show_phase and ax_phase is not None:
                                if show_sdd11:
                                    ax_phase.plot(f, sdd11_phase, 'ko')
                                    ax_phase.annotate(f'SDD11 {sdd11_phase:.2f}°', (f, sdd11_phase),
                                                      xytext=(10, 10), textcoords='offset points')
                                if show_sdd21:
                                    ax_phase.plot(f, sdd21_phase, 'ko')
                                    ax_phase.annotate(f'SDD21 {sdd21_phase:.2f}°', (f, sdd21_phase),
                                                      xytext=(10, 10), textcoords='offset points')
                                if show_s11:
                                    ax_phase.plot(f, s11_phase_val, 'ko')
                                    ax_phase.annotate(f'S11 {s11_phase_val:.2f}°', (f, s11_phase_val),
                                                      xytext=(10, 10), textcoords='offset points')
                                if show_s21:
                                    ax_phase.plot(f, s21_phase_val, 'ko')
                                    ax_phase.annotate(f'S21 {s21_phase_val:.2f}°', (f, s21_phase_val),
                                                      xytext=(10, 10), textcoords='offset points')
                                # S33/S43 phase if enabled and available
                                if self.plot_s33_var.get() and net.s.shape[1] >= 3:
                                    s33_phase_val = np.angle(net.s[idx, 2, 2], deg=True)
                                    ax_phase.plot(f, s33_phase_val, 'ko')
                                    ax_phase.annotate(f'S33 {s33_phase_val:.2f}°', (f, s33_phase_val),
                                                      xytext=(10, 10), textcoords='offset points')
                                if self.plot_s43_var.get() and net.s.shape[1] >= 4:
                                    s43_phase_val = np.angle(net.s[idx, 3, 2], deg=True)
                                    ax_phase.plot(f, s43_phase_val, 'ko')
                                    ax_phase.annotate(f'S43 {s43_phase_val:.2f}°', (f, s43_phase_val),
                                                      xytext=(10, 10), textcoords='offset points')
                                # Arbitrary Sij phase markers
                                if getattr(self, 'custom_sij', None):
                                    for (ii, jj) in self.custom_sij:
                                        if 1 <= ii <= net.s.shape[1] and 1 <= jj <= net.s.shape[2]:
                                            sij_phase = np.angle(net.s[idx, ii-1, jj-1], deg=True)
                                            ax_phase.plot(f, sij_phase, 'ko')
                                            ax_phase.annotate(f'S{ii}{jj} {sij_phase:.2f}°', (f, sij_phase),
                                                              xytext=(10, 10), textcoords='offset points')
                        else:
                            # Non-overlay: use individual axes map
                            if show_mag:
                                if show_sdd11:
                                    ax = ax_map['sdd11_mag']
