...
def main():
    ...
    def _selected_initial_prompt() -> str | None:
        # Provide an English bias prompt only when English is explicitly selected
        try:
            if _selected_language_code() == 'en':
                return "Transcribe the following English speech:"
        except Exception:
            pass
        return None
    ...
    def compute_transcribe_args() -> dict:
        ...
        # Default: transcribe (no translation)
        args = dict(fp16=False, verbose=True, language=lang, task='transcribe', temperature=0, initial_prompt=init_prompt)
        if mode == "English (Translate)":
            # Whisper's translate mode outputs English
            args.update(task='translate', language='en', initial_prompt="Transcribe the following English speech:")
        elif mode == "Chinese (Translate)":
            # Keep task=transcribe in source language; we'll post-translate to Chinese
            pass
        return args
    ...
    # --- Menubar with Tools -> Clear Cache ---
    try:
