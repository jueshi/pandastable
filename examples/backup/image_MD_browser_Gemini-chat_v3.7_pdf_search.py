...
class ImageBrowser(tk.Tk):
    ...
    def _render_syntax_highlighted(self, code: str, width: int, height: int, title: str = ""):
        ...
        try:
            ...
            # Configure formatter with appropriate styling
            ...
            # Generate highlighted code as PNG
    ...
    def _render_text_snapshot(self, text: str, width: int, height: int, title: str = "", force_plain: bool = False):
        ...
        """
        # Check if this is a Python file and we have Pygments available
        if not force_plain and title.lower().endswith('.py'):
            try:
                # Try to import pygments if not already available
                if 'pygments' not in globals():
                    from pygments import highlight
                    from pygments.lexers import get_lexer_by_name
                    from pygments.formatters import ImageFormatter
