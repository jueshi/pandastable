...
def main():
    ...
    def extract_audio_to_wav(input_path: str, out_path: str) -> bool:
        """Extract mono 16 kHz WAV from media using ffmpeg. Return True on success."""
        try:
        ...
        def worker():
            try:
                ...
                for idx, iid in enumerate(order, start=1):
                    ...
                    ok = extract_audio_to_wav(media_path, out_wav)
                    if ok:
            ...
            try:
                ...
                for idx, iid in enumerate(order, start=1):
                    ...
                    try:
                        ...
                        if stop_event.is_set():
                            break
                        result = model.transcribe(wav_path, **args)
                        if stop_event.is_set():
                            break
                        transcript = result.get("text", "").strip()
