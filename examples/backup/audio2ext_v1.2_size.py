...
def main():
    ...
    tree.heading("audio", text="Audio File", command=lambda c="audio": tree_sort_by(c))
    tree.heading("size", text="Size", command=lambda c="size": tree_sort_by(c))
    tree.heading("modified", text="Date Modified", command=lambda c="modified": tree_sort_by(c))
    tree.heading("transcript", text="Transcript .txt", command=lambda c="transcript": tree_sort_by(c))
    tree.column("audio", width=360, anchor="w")
    tree.column("size", width=90, anchor="e")
    tree.column("modified", width=160, anchor="w")
    tree.column("transcript", width=260, anchor="w")
    ...
    def populate_tree(folder: str):
        ...
        for name in sorted(os.listdir(folder)):
            ...
            if ext.lower() in audio_exts:
                ...
                entries.append((name, full, size_str, modified_str, transcript))

        for name, full, size_str, modified_str, transcript in entries:
            iid = tree.insert("", tk.END, values=(name, size_str, modified_str, transcript))
    ...
    def tree_rename_pair():
        ...
        try:
            ...
            if old_audio_path in path_to_item:
                ...
                try:
                    new_size_str = human_readable_size(os.path.getsize(new_audio_path))
                except Exception:
                    new_size_str = "-"
                try:
                    new_mtime_str = human_readable_mtime(os.path.getmtime(new_audio_path))
                except Exception:
                    new_mtime_str = "-"
                tree.item(iid, values=(new_audio_name, new_size_str, new_mtime_str, new_txt_path if os.path.exists(new_txt_path) else "(missing)"))
                tree.focus(iid)
