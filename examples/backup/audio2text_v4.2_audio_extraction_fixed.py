...
def main():
    ...
    tk.Label(frame, text="Stream:").pack(side=tk.LEFT, padx=(8, 2))
    ...
    def do_transcribe(audio_path: str):
        try:
            ...
            # Build args and transcribe via CLI (killable)
            args = compute_transcribe_args()
            try:
                transcript, produced_txt = transcribe_with_cli(input_for_whisper, current_model_name.get().strip() or "small", args)
            except RuntimeError as e:
            ...
            # Prefer produced file if CLI wrote one; otherwise write our own
            output_file = produced_txt or os.path.join(folder, f"{base}.txt")
            if not produced_txt:
            ...
            try:
                for idx, audio_path in enumerate(file_list, start=1):
                    ...
                    try:
                        ...
                        args = compute_transcribe_args()
                        try:
                            transcript, produced_txt = transcribe_with_cli(audio_path, current_model_name.get().strip() or "small", args)
                        except RuntimeError as e:
                            if "canceled by user" in str(e).lower():
                                set_status("Canceled by user.")
                                append_log("Transcription canceled.")
                ...
                for idx, iid in enumerate(order, start=1):
                    ...
                    try:
                        ...
                        if stop_event.is_set():
                            break
                        try:
                            transcript, produced_txt = transcribe_with_cli(wav_path, current_model_name.get().strip() or "small", args)
                        except RuntimeError as e:
                            if "canceled by user" in str(e).lower():
                                set_status("Canceled by user.")
                                append_log("Transcription canceled.")
                        ...
                        base, _ = os.path.splitext(os.path.basename(media_path))
                        output_file = produced_txt or os.path.join(folder_local, f"{base}.txt")
                        if not produced_txt:
                            with open(output_file, "w", encoding="utf-8") as f:
                                f.write(transcript)
                        output_path_var.set(output_file)
