...
def main():
    ...
    tk.Label(frame, text="Model:").pack(side=tk.LEFT, padx=(8, 2))
    ...
    def on_stop():
        ...
        try:
            ...
            pass
    stop_btn = tk.Button(frame, text="Stop", width=8, command=on_stop)
    stop_btn.pack(side=tk.LEFT, padx=(8, 0))

    # Language selector: Auto or pick a specific Whisper language
    tk.Label(frame, text="Language:").pack(side=tk.LEFT, padx=(8, 2))
    ...
    tk.Checkbutton(frame, text="Indian English bias", variable=indian_bias_var).pack(side=tk.LEFT, padx=(8, 0))

    # Stability Mode: enable beam search for steadier output (slower)
    stability_var = tk.BooleanVar(value=False)
    tk.Checkbutton(frame, text="Stability Mode (beam)", variable=stability_var).pack(side=tk.LEFT, padx=(8, 0))

    # Fast decode: greedy, temperature=0 (overrides Stability when ON)
    fast_decode_var = tk.BooleanVar(value=False)
    tk.Checkbutton(frame, text="Fast Decode", variable=fast_decode_var).pack(side=tk.LEFT, padx=(8, 0))
    ...
    tk.Label(frame, text="Output:").pack(side=tk.LEFT, padx=(8, 2))
    ...
    def update_recent_combo_width(candidates: list[str] | None = None):
        try:
            # Determine the longest string width in pixels
            font_obj = tkfont.nametofont(str(recent_combo.cget("font"))) if recent_combo.cget("font") else tkfont.nametofont("TkDefaultFont")
        except Exception:
            font_obj = tkfont.nametofont("TkDefaultFont")
        try:
            vals = candidates if candidates is not None else list(recent_combo["values"]) if recent_combo["values"] else []
        except Exception:
            vals = []
    ...
    # Show current model in status area
    model_display_var = tk.StringVar(value=f"Model: {current_model_name.get()}")
    model_status_label = tk.Label(root, textvariable=model_display_var, anchor="w", fg="#666")
    model_status_label.pack(fill=tk.X, padx=8, pady=(0, 4))
    ...
    # Legend for icons in the Audio column
    legend_lbl = tk.Label(files_frame, text="Legend: 🎥 video", fg="#666", anchor="w")
    legend_lbl.pack(side=tk.TOP, fill=tk.X, padx=(0, 0), pady=(0, 2))
    ...
    def set_recent_values(values: list[str]):
        ...
        try:
            update_recent_combo_width(values)
        except Exception:
            pass
    ...
    def sanitize_notes_markdown(text: str) -> str:
        ...
        try:
            ...
            try:
                ...
                pass
            # Remove lines that are just the marker or markers embedded inline
            # Patterns: [cite_start], [CITE_END], [cite_start ], [ cite_end]
            t = re.sub(r"\[\s*cite_\s*start\s*\]", "", t, flags=re.IGNORECASE)
            t = re.sub(r"\[\s*cite_\s*end\s*\]", "", t, flags=re.IGNORECASE)
    ...
    def render_markdown_to_preview(text: str):
        ...
        if 'HtmlFrame' in locals() and HtmlFrame is not None:
            try:
                # Show a friendly placeholder for empty content (preview only)
                display_text = sanitized_input if sanitized_input.strip() else "# Notes\n\nWrite your notes here. Click 'Save Notes' to create the file."
                exts = [
            ...
            try:
                ...
                styled = f"<div style=\"{container_style}\">{html}</div>"
