...
class SParamBrowser(tk.Tk):
    ...
    def plot_sparam(self, freq, s_params, n_ports):
        ...
        if n_ports == 4:
            ...
            # Create subplots with 1 or 2 rows and N columns (selected params)
            ...
            for idx, (name, mag_vals, phase_vals) in enumerate(selected_params):
                if show_mag:
                    ...
                    for marker_freq in self.markers:
                        m_idx = np.abs(freq - marker_freq).argmin()
                        ax_mag.plot(marker_freq/1e9, mag_vals[m_idx], 'ro', markersize=8)
                        ax_mag.annotate(f'{marker_freq/1e9:.3f} GHz\n{mag_vals[m_idx]:.2f} dB',
                                        (marker_freq/1e9, mag_vals[m_idx]), xytext=(10, 10),
                ...
                if show_phase:
                    ...
                    for marker_freq in self.markers:
                        m_idx = np.abs(freq - marker_freq).argmin()
                        ax_phase.plot(marker_freq/1e9, phase_vals[m_idx], 'ro', markersize=8)
                        ax_phase.annotate(f'{marker_freq/1e9:.3f} GHz\n{phase_vals[m_idx]:.2f}°',
                                          (marker_freq/1e9, phase_vals[m_idx]), xytext=(10, 10),
                                          textcoords='offset points', ha='left')
    ...
    def plot_network_params(self, *networks, show_mag=True, show_phase=True):
        ...
        try:
            ...
            if show_tdr or show_pulse or show_impedance or show_impedance_time:
                ...
                n_plots = 0
                if show_mag:
                    n_plots += (show_sdd11 + show_sdd21 + show_s11 + show_s21)
                if show_phase:
                    n_plots += (show_sdd11 + show_sdd21 + show_s11 + show_s21)
                ...
                if show_mag and show_s11:
                    ax_map['s11_mag'] = axes[ax_idx]; ax_idx += 1
                if show_mag and show_s21:
                ...
                if show_phase and show_s11:
                    ax_map['s11_phase'] = axes[ax_idx]; ax_idx += 1
                if show_phase and show_s21:
