...
def main():
    ...
    def render_markdown_to_preview(text: str):
        ...
        if 'HtmlFrame' in locals() and HtmlFrame is not None:
            try:
                display_text = text if (isinstance(text, str) and text.strip()) else "# Notes\n\nWrite your notes here. Click 'Save Notes' to create the file."
                exts = [
            ...
            try:
                ...
                md_preview.insert(tk.END, text if (isinstance(text, str) and text.strip()) else "(Empty notes)")
