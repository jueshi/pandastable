...
def main():
    ...
    tree.heading("audio", text=header_labels["audio"], command=lambda c="audio": tree_sort_by(c))
    tree.heading("size", text=header_labels["size"], command=lambda c="size": tree_sort_by(c))
    tree.heading("modified", text=header_labels["modified"], command=lambda c="modified": tree_sort_by(c))
    tree.heading("transcript", text=header_labels["transcript"], command=lambda c="transcript": tree_sort_by(c))
    ...
    # Horizontal scrollbar to support long transcript paths
    h_scroll = ttk.Scrollbar(files_frame, orient=tk.HORIZONTAL, command=tree.xview)
                ...
                if item_id:
                    ...
                    try:
                        autofit_tree_columns(
                            tree,
                            cols,
                            min_widths={"size": 80, "modified": 140},
                            max_widths={"transcript": 600},
                            padding=18,
                        )
                    except Exception:
                        pass
