...
class XMLViewer:
    def __init__(self, root):
        ...
        # self.notes_dir = r"C:\Users\JueShi\OneDrive - Astera Labs, Inc\Documents\reg_maps"
        self.notes_dir = r"C:\Users\juesh\OneDrive\Documents\reg_maps"
        try:
        ...
        self.note_text = scrolledtext.ScrolledText(
            notes_bottom, wrap=tk.WORD, width=40, height=8
        )
        # Allow the notes pane to expand; sash makes it user-resizable
        self.note_text.pack(fill=tk.BOTH, expand=True, padx=5, pady=(0,4))
        ...
        for module, regs in modules.items():
            ...
            for reg, fields in regs.items():
                ...
                try:
                    ...
                    addr_int = None
                    addr_end_int = None
                rid = self.tree.insert(mid, 'end', text=f"Register {reglabel} @ {addr}", open=False, tags=('register',))
                self._node_meta[rid] = {
    ...
    def save_to_csv(self):
        ...
        try:
            ...
            messagebox.showinfo('Success', f'Saved CSV to {save_path}')
        except Exception as e:
    ...
    def _note_key(self, reg: dict) -> str:
        ...
        block_name = (reg.get('block') or '').strip()
        ...
        try:
            ...
            if isinstance(meta, dict):
                ...
                for r in self.registers:
                    if r.get('name','') == mname:
                        return r
                # Synthesize a minimal register record for notes
                # Try to derive block/map from ancestors
                block = ''
                map_name = ''
                # Walk up ancestors to collect labels
                anc = []
                pid = self.tree.parent(item_id)
                while pid:
                    it = self.tree.item(pid)
                    anc.append(it.get('text',''))
                    pid = self.tree.parent(pid)
                # Detect SNPS "Module X" style
                for label in anc:
                    if isinstance(label, str) and label.startswith('Module '):
                        block = label.split(' ',1)[1]
                        map_name = ''
                        break
                # Legacy path: last two ancestors are block and map
                if not block and len(anc) >= 2:
                    block = anc[0]
                    map_name = anc[1]
                return {'name': mname, 'map': map_name, 'block': block, 'fields': []}
            # Fallback 2: search downward from current selection for first register child
            # Useful when user selects module/map node and immediately hits Save Note
            stack = list(self.tree.get_children(item_id)) if item_id else []
            while stack:
                cid = stack.pop(0)
                it = self.tree.item(cid)
                if 'register' in it.get('tags', ()):  # found a register node
                    rtext = it.get('text','')
                    rname = rtext
                    if rtext.startswith('Register '):
                        try:
                            rname = rtext.split(' ',1)[1].split(' @ ',1)[0]
                        except Exception:
                            rname = rtext
                    for r in self.registers:
                        if r.get('name','') == rname:
                            return r
                    # synthesize minimal record
                    return {'name': rname, 'map': '', 'block': '', 'fields': []}
                # breadth-first
                stack.extend(self.tree.get_children(cid))
        except Exception:
            return None
        return None
    ...
    def _note_file_for_register(self, reg: dict) -> str:
        ...
        module = (reg.get('block') or '').strip() or self._infer_module_from_selection() or 'module'
        rname = (reg.get('name') or '').strip() or 'register'
    ...
    def save_current_note(self):
        ...
        self._status(f"Saved note for {key} → {dest}")
            ...
            for block_name, regs in blocks.items():
                ...
                for reg in regs:
                    ...
                    reg_node = self.tree.insert(
                        ...
                        values=(f"0x{reg_offset_int:X}", reg.get('size', ''), reg.get('access', '')), tags=('register','match')
                    )
        ...
        try:
            ...
            for module, regs in (modules or {}).items():
                ...
                for reg, info in regs.items():
                    ...
                    rid = self.tree.insert(mid, 'end', text=f"Register {reg} @ {address}", open=False)
                    self._node_meta[rid] = {
    ...
    def save_matches_to_csv(self):
        ...
        if not subset:
            ...
            return
        # Options
        opt = { 'add_compact': tk.BooleanVar(value=True) }
            ...
            with open(file_path, 'w', newline='', encoding='utf-8') as csvfile:
                ...
                for reg in subset:
                    ...
                    for f in reg.get('fields', []):
                        ...
                        writer.writerow(row)
            messagebox.showinfo("Success", f"Matched register map successfully saved to {file_path}")
        except Exception as e:
                    ...
                    for rname, reg_list in regs.items():
                        ...
                        if not fields:
                            ...
                            for f in fields:
                                ...
                                w.writerow(row)
            messagebox.showinfo("Success", f"Saved (SNPS) CSV: {file_path}")
        except Exception as e:
    ...
    def convert_xml_to_csv(self):
        ...
        if not xml_path:
            return
        # Options dialog
        opts = { 'skip_fields': tk.BooleanVar(value=False), 'parallel': tk.BooleanVar(value=False) }
        dlg = tk.Toplevel(self.root)
        ...
        try:
            ...
            try:
                with open(xml_path, 'r', encoding='utf-8-sig') as f:
                    content = f.read()
                # Remove any BOM and leading whitespace that can confuse the parser
                content = content.lstrip('\ufeff').lstrip()
                # If content is HTML-escaped XML, unescape first
                if any(ent in content for ent in ('&lt;', '&gt;', '&amp;')):
                    content = html.unescape(content)
                # Safely escape stray ampersands that are not part of a valid entity
                import re as _re
                content = _re.sub(r'&(?!#\d+;|#x[0-9A-Fa-f]+;|[A-Za-z][A-Za-z0-9]*;)', '&amp;', content)
                root = ET.fromstring(content)
                local_root = root.tag.split('}', 1)[-1] if '}' in root.tag else root.tag
            except ET.ParseError as pe:
                preview = (content[:200].replace('\n', ' ') if 'content' in locals() else '')
                raise ET.ParseError(f"{pe}. Preview: {preview}") from pe
            except Exception:
                local_root = ''
            ...
            if parallel:
                ...
                self._parallel_convert_to_csv(xml_path, csv_path, skip_fields)
            else:
                self._stream_write_xml_to_csv(xml_path, csv_path, skip_fields)
            self._status("Done")
            messagebox.showinfo("Success", f"Converted to CSV: {csv_path}")
        except Exception as e:
            self._status("")
    ...
    def _stream_write_xml_to_csv(self, file_path: str, csv_path: str, skip_fields: bool):
    ...
    def _parallel_convert_to_csv(self, file_path: str, csv_path: str, skip_fields: bool):
            ...
            with open(csv_path, 'r', encoding='utf-8') as f:
                ...
                if is_snps:
                    ...
                    for module, regs in modules.items():
                        ...
                        self._node_meta[mid] = {'mode': 'snps', 'kind': 'module', 'name': module}
                        for reg, info in regs.items():
                            rid = self.tree.insert(mid, 'end', text=f"Register {reg} @ {info.get('address','')}", open=False)
                            self._node_meta[rid] = {
                                'mode': 'snps', 'kind': 'register', 'name': reg,
                                'address': info.get('address',''),
                                'reg_width_bits': info.get('reg_width_bits',''),
