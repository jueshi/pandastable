...
class ImageBrowser(tk.Tk):
        ...
        try:
            ...
            if not query:
                ...
                if q.startswith('.'):
                    ...
                    mask = self._base_df['Name'].str.lower().str.endswith(q, na=False)
                else:
    ...
    def _update_pdf_controls(self, label=None):
        ...
        try:
            ...
            if not img_path or not img_path.lower().endswith('.pdf'):
                self._hide_pdf_controls()
                return
                
            # Show the controls if hidden
            if not self.pdf_nav_frame.winfo_ismapped():
                self.pdf_nav_frame.pack(fill=tk.X, padx=5, pady=2)
    ...
    def _render_syntax_highlighted(self, code: str, width: int, height: int, title: str = ""):
        """Render syntax-highlighted code as an image using Pygments.
        ...
        Returns:
            PIL.Image: Rendered image with syntax highlighting
        """
        ...
        try:
            ...
            print("Debug: Creating Python lexer")
            lexer = get_lexer_by_name('python', stripall=True)
            print(f"Debug: Lexer created: {lexer}")
            
            # Get a nice color scheme
            style = get_style_by_name('default')
            
            # Configure formatter with better styling
            formatter = ImageFormatter(
                image_format='PNG',
                style=style,
                line_numbers=False,
                font_name='Consolas',  # Better font for code
                font_size=12,         # Slightly larger font
                line_pad=4,           # More padding between lines
                image_pad=12,         # More padding around the code
                hl_lines=[],          # No highlighted lines (empty list instead of None)
                hl_color='#ffffcc',   # Light yellow highlight color
                line_number_fg='#666666',  # Line number color
                line_number_bg='#f0f0f0',  # Line number background
                line_number_bold=False,
                line_number_italic=False,
                line_number_pad=10,   # Padding for line numbers
            ...
            # Generate highlighted code as PNG with higher resolution
            print("Debug: Generating highlighted code")
            
            # Create a custom formatter with higher DPI for better quality
            formatter = ImageFormatter(
                style='monokai',
                line_numbers=False,
                font_name='Consolas',
                ...
                line_pad=2,
                image_format='png',
                dpi=144  # Higher DPI for better quality
            )
            
            # Generate the highlighted code with the custom formatter
            png_data = highlight(code, lexer, formatter)
            print(f"Debug: PNG data generated, length: {len(png_data) if png_data else 0} bytes")
            
            # Convert PNG data to PIL Image
            print("Debug: Converting to PIL Image")
            img = Image.open(BytesIO(png_data))
            print(f"Debug: Image created: {img.size[0]}x{img.size[1]}")
            
            # Convert to RGB if needed (for better compatibility)
            if img.mode != 'RGB':
                img = img.convert('RGB')
            
            # Calculate scaling to fit the image within the available space
            # Use a minimum scale of 1.0 to prevent upscaling blur
            scale = min(1.0, width / img.width, height / img.height)
            
            # If the image is smaller than the container, don't scale it up
            if scale > 1.0:
                scale = 1.0
            ...
            if scale < 1.0:
                # For downscaling, use LANCZOS (a high-quality downsampling filter)
                img = img.resize((new_width, new_height), Image.Resampling.LANCZOS)
            # If scale is 1.0, keep original size
            
            # Create a new image with the exact requested dimensions
            ...
            # Always paste the image, even if it's larger than the container
            # The image is already scaled to fit
            ...
            return self._render_text_snapshot(code, width, height, title, force_plain=True)
    ...
    def _render_text_snapshot(self, text: str, width: int, height: int, title: str = "", force_plain: bool = False):
        ...
        # Check if this is a Python file and we should try syntax highlighting
        if not force_plain and title and title.lower().endswith('.py'):
            print("Debug: Python file detected, attempting syntax highlighting")
            try:
                # Try to import pygments to check if it's available
                import importlib
                pygments_spec = importlib.util.find_spec('pygments')
                if pygments_spec is not None:
                    return self._render_syntax_highlighted(text, width, height, title)
            except ImportError:
                print("Pygments not installed. Install with: pip install pygments")
            except Exception as e:
                print(f"Error during syntax highlighting: {e}")
                import traceback
                traceback.print_exc()
        
        # Original plain text rendering for non-Python files or if Pygments fails
        from PIL import ImageDraw, ImageFont
