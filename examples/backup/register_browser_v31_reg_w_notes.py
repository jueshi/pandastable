...
class XMLViewer:
    def __init__(self, root):
        ...
        ttk.Separator(md_toolbar, orient=tk.VERTICAL).pack(side=tk.LEFT, fill=tk.Y, padx=(8,6))
        ttk.Button(md_toolbar, text='Notes Only', width=10, command=self._filter_tree_to_noted_registers).pack(side=tk.LEFT)
        ttk.Button(md_toolbar, text='All', width=5, command=self.clear_search).pack(side=tk.LEFT, padx=(4,0))
        ...
        try:
            ...
            for module, regs in (modules or {}).items():
                ...
                for reg, info in regs.items():
                    ...
                    rid = self.tree.insert(mid, 'end', text=f"Register {reg} @ {address}", open=False, tags=('register',))
                    self._node_meta[rid] = {
            ...
            with open(csv_path, 'r', encoding='utf-8') as f:
                ...
                if is_snps:
                    ...
                    for module, regs in modules.items():
                        ...
                        self._node_meta[mid] = {'mode': 'snps', 'kind': 'module', 'name': module}
                        for reg, info in regs.items():
                            rid = self.tree.insert(mid, 'end', text=f"Register {reg} @ {info.get('address','')}", open=False, tags=('register',))
                            self._node_meta[rid] = {
                                'mode': 'snps', 'kind': 'register', 'name': reg,
                                'address': info.get('address',''),
                                'reg_width_bits': info.get('reg_width_bits',''),
