...
class SParamBrowser(tk.Tk):
    ...
    def calculate_tdr(self, network=None, freq_limit=None):
        ...
        # Define sdd11_complex_padded
        sdd11_complex_padded = s11_padded        
        ...
        n_padded = len(sdd11_complex_padded_sym)      
        # === Step 5: Compute time-domain impulse response ===
        # Use IFFT for time domain conversion (standard approach)
        tdr_impulse = np.fft.ifft(sdd11_complex_padded_sym)
        
        # === Step 5.5: Convert impulse response to step response ===
        dt = 1 / (2 * f_padded[-1])  # Time step
        tdr_raw = np.cumsum(tdr_impulse) * dt
