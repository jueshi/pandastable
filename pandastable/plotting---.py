...
# Support running both as part of the pandastable package and as a standalone script
if __package__:
    from .dialogs import *
...
class PlotViewer(Frame):
    ...
    def plot2D(self, redraw=True):
        ...
        # Prevent log-y plots when data has zero/negative values
        if kwds.get('logy'):
            numeric_data = data.select_dtypes(include=['number'])
            if numeric_data.empty or (numeric_data <= 0).any().any():
                kwds['logy'] = False
                self.mplopts.kwds['logy'] = False
                self.showWarning('Log Y disabled: selected data contains zero or negative values')
        ...
        if by != '' and kwargs['subplots'] == False:
            # Grouped overlay on a single axis
            cmap = plt.cm.get_cmap(kwargs['colormap']) if 'colormap' in kwargs else None
            handles = None
            labels = None
            # Iterate through groups and overlay
            n_groups = len(g)
            for i, (n, df) in enumerate(g):
                # drop grouping columns from data to plot
                try:
                    d = df.drop(by, axis=1)
                except Exception:
                ...
                # choose group color
                grp_color = None
                if cmap is not None and n_groups > 0:
                    grp_color = cmap(float(i) / n_groups)

                k = kind
                # supported overlay kinds
                if k in ['line', 'bar', 'barh']:
                    # pass a copy of kwargs and set color per group
                    kkwargs = kwargs.copy()
                    if grp_color is not None:
                        kkwargs['color'] = grp_color
                        # avoid colormap-driven per-series colors for overlay
                        if 'colormap' in kkwargs:
                            del kkwargs['colormap']
                    axs = self._doplot(d, self.ax, k, False, errorbars, useindex=None,
                                       bw=bw, yerr=None, kwargs=kkwargs, label=str(n))
                    handles, labels = self.ax.get_legend_handles_labels()
                elif k == 'scatter':
                    # custom scatter overlay: x is first numeric col, y are others
                    nd = d._get_numeric_data()
                    if nd.shape[1] < 2:
                        continue
                    xcol = nd.columns[0]
                    ycols = nd.columns[1:]
                    ms = kwargs.get('ms', 6) * 12
                    marker = kwargs.get('marker', 'o') or 'o'
                    lw = kwargs.get('linewidth', 1.0)
                    alpha = kwargs.get('alpha', 0.8)
                    for ycol in ycols:
                        self.ax.scatter(nd[xcol], nd[ycol], marker=marker, alpha=alpha,
                                        linewidth=lw, s=ms, edgecolors='black',
                                        facecolor=grp_color, label=f"{ycol} {n}")
                    # apply log/grid for scatter
                    ...
                    self.showWarning('Overlay not supported for kind=%s; enable multiple subplots' % k)
                    return

            axs_list = [self.ax]
        elif kwargs['subplots'] == True and by != '':
            i = 1
            if len(g) > 30:
